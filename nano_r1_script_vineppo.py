...
def create_vineppo_training_episodes(
    ...
    def mc_queries(ds):
        ...
        mc_outputs = inference_engine.generate(
                ...
                sampling_params=[
                    ...
                    top_p=TOP_P,
                    top_k=TOP_K,
                    max_tokens=max(max_tokens, 1), # yes, max(max_tokens,1) makes us violate the max_tokens limit by one occasionally, but fixing it has little benefit while making the code 2x more complicated :) 
                    detokenize=False,
    ...
    all_query_token_ids, all_responses_token_ids, all_responses, all_queries= [], [], [], []
    all_samples = []
    all_rewards = []
    ...
    for sample, group_indices in zip(samples, groups):
        ...
        all_queries.extend([sample["prompt"]] * GENERATIONS_PER_SAMPLE)
        minimal_sample = {'target': sample["target"], 'nums': sample["nums"]}
        all_samples.extend([minimal_sample] * GENERATIONS_PER_SAMPLE)
...
def main():
    ...
    print(f"Test dataset size: {len(test_dataset)}")
     ...
     ############################################
    # Initialize vLLM (Inference) engine; initialize before models until https://github.com/vllm-project/vllm/issues/2248 is fixed, or use v0 engine(set VLLM_USE_V1=0)
    ############################################
    ...
    def fix_oov_logits_processor(tokenizer_vocab_size):
    ...
    logit_processor = fix_oov_logits_processor(len(inference_engine.get_tokenizer().get_vocab())) # remove asap when this is fixed in vllm, it is dirty and even logit processors are not supported in engine v1 of vllm 
